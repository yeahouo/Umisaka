...
import { GitHelper, GitStatus } from './git-utils.js';
  ...
  try {
    switch (name) {
      ...
      case 'git_auto_commit':
        return await handleGitAutoCommit(args?.custom_message as string | undefined);

      case 'git_commit':
        return await handleGitCommit(args?.message as string, args?.push as boolean);

      case 'git_diff':
        return await handleGitDiff(args?.file as string | undefined);
...
// 处理自动提交
async function handleGitAutoCommit(customMessage?: string) {
  const status = await gitHelper.getStatus();
...
// 处理手动提交
async function handleGitCommit(message: string, push: boolean = true) {
  if (!message?.trim()) {
...
// 处理差异查看
async function handleGitDiff(file?: string) {
  try {
